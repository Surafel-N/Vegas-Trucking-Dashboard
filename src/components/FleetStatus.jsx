...
export function FleetStatus({ totalTrips, activeDays, profitableTrips, driverLabel, profitMargin, t, language = "FR" }) {
  const isEn = language === "EN";
  const items = [
    {
      label: t?.activeDays || (isEn ? "Active Days" : "Jours d'Activité"),
      value: activeDays,
      icon: CalendarDays,
      accent: "bg-white/5 text-white/60",
    },
    {
      label: t?.profitableTrips || (isEn ? "Profitable Trips" : "Voyages Profitables"),
    ...
    <section className="panel-enter rounded-[40px] border border-white/5 bg-[#111] p-6 text-white shadow-2xl h-full flex flex-col justify-between">
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-3 mb-4">
           ...
           <div>
              <p className="text-[10px] uppercase tracking-[0.2em] text-[#cf5d56] leading-none mb-1">{t?.fleetSummary || (isEn ? "Fleet Summary" : "Résumé Flotte")}</p>
        ...
        <div className="inline-block w-fit rounded-full border border-white/5 bg-white/5 px-4 py-1.5 text-[10px] font-black uppercase tracking-widest text-white/40">
          {t?.opMargin || (isEn ? "Operating Margin" : "Marge Opérationnelle")}: <span className="text-white ml-1">{profitMargin}</span>
        </div>
      </div>
      ...
      <div className="mt-6 flex items-center gap-3 rounded-[25px] border border-[#cf5d56]/10 bg-[#cf5d56]/5 p-4 text-[11px] font-medium text-white/40">
        ...
        {t?.currentFocus || (isEn ? "Current Focus" : "Focus Actuel")}: <span className="font-black text-white uppercase tracking-tighter">{driverLabel}</span>
      </div>
    </section>
  );
}
